...
public class OuterProductRepository {
	...
	public OuterProduct getOuterProductById(String OuterProductID) {
		OuterProduct productById = null;
		for (int i = 0; i < listOfProducts.size(); i++) {
			OuterProduct product = listOfProducts.get(i);
			if (product != null && product.getOuterProductID() != null
					&& product.getOuterProductID().equals(OuterProductID)) {
				productById = product;
				break;
			}
		}
